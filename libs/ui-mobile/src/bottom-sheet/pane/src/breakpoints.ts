...
export class Breakpoints {
  ...
  public async buildBreakpoints(
    ...
    if (this.instance.isPanePresented()) {
      ...
      if (!this.settings.breaks[this.prevBreakpoint]?.enabled) {
        if (!this.instance.isHidden()) {
          let nextY = this.instance.swipeNextPoint(
            1,
            1,
            this.getClosestBreakY(),
          );
          const nextBreak = Object.entries(this.breaks).find(
            (val) => val[1] === nextY,
          );
  ...
  public getClosestBreakY(): number {
    return this.brs.reduce((prev, curr) => {
      return Math.abs(curr - this.instance.getPanelTransformY()) <
        Math.abs(prev - this.instance.getPanelTransformY())
        ? curr
        : prev;
    });
  }
