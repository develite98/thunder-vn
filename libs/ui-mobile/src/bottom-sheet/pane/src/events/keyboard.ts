...
export class KeyboardEvents {
  ...
  private async onKeyboardShow(e) {
    ...
    let currentHeight =
      (this.instance.getPanelTransformY() - this.instance.screen_height) * -1;
    const inputEl = document.activeElement;
    const inputElBottomBound: number = this.getActiveInputClientBottomRect();
    const inputSpaceBelow =
      this.instance.screen_height - inputElBottomBound - this.inputBottomOffset;
    ...
    if (e.keyboardHeight > inputSpaceBelow) {
      ...
      const newInputBottomOffset: number =
        this.getActiveInputClientBottomRect();
  ...
  private onKeyboardWillHide(e) {
    ...
    if (
      ...
      this.breakpoints.breaks[this.breakpoints.prevBreakpoint]
    ) {
      return;
    }
  ...
  private getActiveInputClientBottomRect(): number {
    if (
      document.activeElement.classList.contains('native-textarea') ||
  ...
  public fixBodyKeyboardResize(showKeyboard) {
    ...
    //       'min-height',
    //       `${this.instance.screen_height}px`,
    //     );
