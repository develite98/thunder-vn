...
export class Events {
  ...
  constructor(private instance: CupertinoPane) {
    ...
    this.swipeNextSensivity = window.hasOwnProperty('cordova')
      ? this.settings.fastSwipeSensivity + 2
  ...
  private getTouchEvents(): {
    ...
    return Support.touch || !this.settings.simulateTouch
      ? touchEventsTouch
      : touchEventsDesktop;
  }
  ...
  public attachAllEvents() {
    ...
    if (this.settings.handleKeyboard && this.device.cordova) {
      window.addEventListener(
        'keyboardWillShow',
        this.keyboardEvents.onKeyboardShowCb,
      );
      window.addEventListener(
        'keyboardWillHide',
        this.keyboardEvents.onKeyboardWillHideCb,
      );
    }
  ...
  private touchMove(t) {
     ...
     * TODO: Switch to pointer events
     */
    const { clientY, clientX, velocityY } = this.getEventClientYX(
      t,
      'touchmove',
    );
    if (!clientY || !clientX) {
    ...
    // textarea scrollbar
    if (this.isFormElement(t.target) && this.isElementScrollable(t.target)) {
      return;
    }
    ...
    if (this.steps.length < 2) {
      ...
      let transitionYDiff =
        computedTranslateY - this.instance.getPanelTransformY();
    ...
    if (this.settings.touchAngle && !this.isScrolling) {
      let touchAngle;
    ...
    // No changes Y/X
    if (
      this.instance.getPanelTransformY() === newVal &&
    ...
    if (
      ...
      if (
        mousePointY <=
        this.instance.screen_height - this.breakpoints.bottomer
      ) {
  ...
  private touchEnd(t) {
    ...
    if (this.fastSwipeNext('Y')) {
      closest = this.instance.swipeNextPoint(
        this.steps[this.steps.length - 1]?.posY -
          this.steps[this.steps.length - 2]?.posY, //diff
  ...
  private onClick(t) {
    ...
    if (this.settings.clickBottomOpen) {
      ...
      if (
        this.breakpoints.breaks['bottom'] === this.instance.getPanelTransformY()
      ) {
        let closest;
  ...
  public fastSwipeNext(axis: 'Y' | 'X'): boolean {
    const diff =
      this.steps[this.steps.length - 1]?.['pos' + axis] -
      this.steps[this.steps.length - 2]?.['pos' + axis];
  ...
  private handleSuperposition(coords: {
    ...
    if (
      this.settings.upperThanTop &&
      (coords.newVal <= this.breakpoints.topper || this.startPointOverTop)
    ) {
      ...
      const screenDelta =
        this.instance.screen_height - this.instance.screenHeightOffset;
      const differKoef =
        (screenDelta - this.instance.getPanelTransformY()) /
        (screenDelta - this.breakpoints.topper) /
        8;
      return {
        y: this.instance.getPanelTransformY() + coords.diffY * differKoef,
      };
    }
  ...
  private getEventClientYX(ev, name) {
    const targetTouch =
      ev.type === name &&
      ev.targetTouches &&
      (ev.targetTouches[0] || ev.changedTouches[0]);
    const clientY: number =
      ev.type === name ? targetTouch?.clientY : ev.clientY;
    const clientX: number =
      ev.type === name ? targetTouch?.clientX : ev.clientX;
    const timeDiff: number =
      Date.now() - (this.steps[this.steps.length - 1]?.time || 0);
    const distanceY: number = Math.abs(
      clientY - (this.steps[this.steps.length - 1]?.posY || 0),
    );
  ...
  private isDraggableElement(t) {
    return (
      t.target === this.instance.draggableEl ||
  ...
  private isFormElement(el): boolean {
    ...
    if (el && el.tagName && formElements.includes(el.tagName.toLowerCase())) {
      return true;
    }
    return false;
  }
