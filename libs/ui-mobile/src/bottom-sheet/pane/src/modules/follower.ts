...
export class FollowerModule {
  ...
  constructor(private instance: CupertinoPane) {
    ...
    this.instance.on('rendered', () => {
      if (
        !(<HTMLElement>document.querySelector(this.settings.followerElement))
      ) {
        console.warn(
      ...
      this.followerEl = <HTMLElement>(
        document.querySelector(this.settings.followerElement)
      );
      this.followerEl.style.willChange = 'transform, border-radius';
      this.followerEl.style.transform = `translateY(0px) translateZ(0px)`;
      this.followerEl.style.transition = this.transitions.buildTransitionValue(
        this.settings.breaks[this.instance.currentBreak()]?.bounce,
      );
    });
