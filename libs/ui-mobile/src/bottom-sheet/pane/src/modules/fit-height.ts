...
export class FitHeightModule {
  ...
  constructor(private instance: CupertinoPane) {
    ...
    this.instance['calcFitHeight'] = async (animated) =>
    ...
    this.instance.on(
      ...
      (ev) => {
        ...
        if (this.settings.fitScreenHeight) {
          if (
            this.settings.breaks[ev.break]?.height > this.instance.screen_height
          ) {
        ...
        if (ev.break === 'top') {
          if (
            this.settings.breaks['top'].height > this.instance.screen_height
          ) {
  ...
  private async beforeBuildBreakpoints(): Promise<void> {
    ...
    this.breakpoints.conf.bottom = this.settings.breaks?.bottom || {
      enabled: true,
      height: 0,
    };
  }
  ...
  private async getPaneFitHeight(): Promise<number> {
      ...
      promises = Array.from(images).map(
        (image) =>
          new Promise((resolve) => {
            ...
            if (image.height || (image.complete && image.naturalHeight)) {
              return resolve(true);
            }
    ...
    let newPaneElHeight = Math.floor(
      this.instance.paneEl.getBoundingClientRect().height,
    );
