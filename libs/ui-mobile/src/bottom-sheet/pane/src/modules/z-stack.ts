...
export class ZStackModule {
  ...
  constructor(private instance: CupertinoPane) {
    ...
    this.instance['setZstackConfig'] = async (zStack: ZStackSettings) =>
      this.setZstackConfig(zStack);
  ...
  public setZstackConfig(zStack: ZStackSettings): void {
    ...
    this.settings.zStack = zStack
      ? { ...this.zStackDefaults, ...zStack }
      : null;
  }
  ...
  public pushTransition(
    ...
    pushElement.style.overflow =
      this.settings.zStack.cardBorderRadius && 'hidden';
    ...
    const topHeight = this.settings.zStack.minPushHeight
      ...
      : this.instance.screenHeightOffset - this.breakpoints.bottomer;
    const minHeight =
      this.instance.screenHeightOffset - this.breakpoints.topper;
    ...
    let scaleNormal = Math.pow(
      this.settings.zStack.cardZScale,
      multiplicator - 1,
    );
    ...
    let contrastNew = Math.pow(
      this.settings.zStack.cardContrast,
      multiplicator,
    );
    let contrastNormal = Math.pow(
      this.settings.zStack.cardContrast,
      multiplicator - 1,
    );
    ...
    const setStyles = (scale, y, contrast, border) => {
      let exponentAngle = Math.pow(
        scale,
        this.settings.zStack.stackZAngle / 100,
      );
  ...
  private getPushMulitplicator(el: HTMLElement): number {
    let multiplicator: string | number = el.style.getPropertyValue(
      '--push-multiplicator',
    );
