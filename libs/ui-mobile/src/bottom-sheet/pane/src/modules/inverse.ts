...
export class InverseModule {
  ...
  constructor(private instance: CupertinoPane) {
    ...
    this.instance['setOverflowHeight'] = () =>
      this.settings.fitHeight ? {} : this.setOverflowHeight();
    this.instance['checkOpacityAttr'] = () => {};
    this.instance['checkOverflowAttr'] = (val) => this.checkOverflowAttr(val);
    this.instance['prepareBreaksSwipeNextPoint'] = () =>
      this.prepareBreaksSwipeNextPoint();
    ...
    this.instance.on(
      ...
      (ev) => {
        if (this.settings.breaks[ev.break]?.enabled) {
          this.breakpoints.breaks[ev.break] =
            2 *
  ...
  public setOverflowHeight() {
    this.instance.overflowEl.style.height = `${
      this.getPaneHeight() -
      30 -
  ...
  public checkOverflowAttr(val) {
    if (!this.settings.topperOverflow || !this.instance.overflowEl) {
      return;
    }

    this.instance.overflowEl.style.overflowY =
      val >= this.breakpoints.bottomer ? 'auto' : 'hidden';
  }
  ...
  public scrollPreventDrag(t): boolean {
    ...
    if (this.events.willScrolled() && this.isOverflowEl(t.target)) {
      prevention = true;
    }
    return prevention;
  }
