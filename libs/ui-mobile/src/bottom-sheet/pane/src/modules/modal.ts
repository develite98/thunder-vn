...
export class ModalModule {
  ...
  constructor(private instance: CupertinoPane) {
    ...
    // set defaults
    this.settings.modal =
      typeof this.settings.modal === 'object'
    ...
    // re-bind functions
    this.events['handleSuperposition'] = (coords) =>
      this.handleSuperposition(coords);
    this.transitions['setPaneElTransform'] = (params) =>
  ...
  private present(conf) {
    ...
    if (!transition) {
      transition =
        ModalModule.BuildInTransition[this.settings.modal['transition']];
    }
  ...
  private destroy(conf) {
      ...
      transition = JSON.parse(
        JSON.stringify({
          duration:
            ModalModule.BuildInTransition[this.settings.modal['transition']]
              .duration,
          from: ModalModule.BuildInTransition[this.settings.modal['transition']]
            .to,
          to: ModalModule.BuildInTransition[this.settings.modal['transition']]
            .from,
        }),
      );
    }
    ...
    if (conf.fromCurrentPosition) {
      let computedTranslate = new WebKitCSSMatrix(
  ...
  private handleSuperposition(coords: {
    ...
    // dismissOnIntense
    let intenseKoeff = 40;
    let differY = Math.abs(
      this.instance.getPanelTransformY() - this.breakpoints.topper,
    );
    let differX = Math.abs(this.instance.getPanelTransformX());
    if (
      this.settings.modal['dismissOnIntense'] &&
    ...
    let hardness = 8;
    const differKoefY =
      this.instance.getPanelTransformY() / this.breakpoints.topper / hardness;
    const differKoefX =
      this.instance.getPanelTransformX() / this.breakpoints.topper / hardness;
    return {
      y:
        this.instance.getPanelTransformY() +
        coords.diffY * (differKoefY + differKoefX),
      x:
        this.instance.getPanelTransformX() +
        coords.diffX * (differKoefY + differKoefX),
    };
  }
}
