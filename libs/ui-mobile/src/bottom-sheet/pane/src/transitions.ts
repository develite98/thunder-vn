  ...
  public doTransition(params: any = {}): Promise<true> {
    return new Promise(async (resolve) => {
      ...
      if (params.type === CupertinoTransition.Move) {
        // System event
        this.instance.emit('onMoveTransitionStart', {
      ...
      const transitionEnd = () => {
        ...
        if (
          params.type === CupertinoTransition.Breakpoint ||
        ...
        this.instance.emit('onTransitionEnd', {
          type: params.type,
          target: document.body.contains(this.instance.paneEl)
            ? this.instance.paneEl
            : null,
        });

        // Remove listener
        this.instance.paneEl.removeEventListener(
          'transitionend',
          transitionEnd,
        );
        return resolve(true);
      ...
      if (
        ...
        // transition style
        let buildedTransition = this.buildTransitionValue(
          bounce,
          subTransition.duration,
        );
        ...
        let getNextBreakpoint = Object.entries(this.breakpoints.breaks).find(
          (val) => val[1] === params.translateY,
        );
