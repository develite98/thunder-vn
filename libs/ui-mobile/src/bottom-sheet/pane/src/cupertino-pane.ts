...
import { emit, on } from './events-emitter';
...
export class CupertinoPane {
  ...
  public calcFitHeight: (animated?: any) => Promise<any> = () => {
    if (!this.settings.fitHeight) {
      console.warn(
        `Cupertino Pane: calcFitHeight() should be used for auto-height panes with enabled fitHeight option`,
      );
      return null;
    }
  };
  ...
  constructor(
    ...
    modules.forEach((module) =>
      !!module.CollectSettings
        ? (this.settings = module.CollectSettings(this.settings))
        : null,
    );
    ...
    if (this.settings.parentElement) {
      parentElement =
        this.settings.parentElement instanceof HTMLElement
    ...
    if (this.settings.events) {
      Object.keys(this.settings.events).forEach((name) =>
        this.on(name, this.settings.events[name]),
      );
    }
    ...
    modules.forEach(
      (module) =>
        (this.modules[this.getModuleRef(module.name)] = new module(this)),
    );
  }
  ...
  async present(
    ...
    if (!this.el || !document.body.contains(this.el)) {
      console.warn(
        'Cupertino Pane: specified DOM element must be attached to the DOM',
      );
      return;
    }
    ...
    let customTransitionFrom = conf?.transition?.from
      ? JSON.parse(JSON.stringify(conf.transition.from))
      : null;
    ...
    if (this.settings.buttonDestroy) {
      ...
      this.destroyButtonEl.addEventListener('click', (t) =>
        this.destroy({ animate: true, destroyButton: true }),
      );
    ...
    if (this.device.ionic && !this.settings.ionContentScroll) {
  ...
  public getPaneHeight(): number {
    return (
      this.screen_height - this.breakpoints.topper - this.settings.bottomOffset
    );
  }

  public updateScreenHeights(): void {
    const visualHeight = localStorage.getItem('appInnerHeight');
    if (visualHeight) {
      this.screen_height = parseInt(visualHeight, 10);
    ...
    this.screenHeightOffset = height;
  }
  ...
  public scrollElementInit() {
    ...
    if (this.settings.topperOverflow && this.settings.upperThanTop) {
      console.warn(
  ...
  public checkOpacityAttr(val) {
    ...
    attrElements.forEach((item) => {
      (<HTMLElement>item).style.transition =
        `opacity ${this.settings.animationDuration}ms ${this.settings.animationType} 0s`;
      (<HTMLElement>item).style.opacity =
        val >= this.breakpoints.breaks['bottom'] ? '0' : '1';
    });
  }

  public checkOverflowAttr(val) {
    if (!this.settings.topperOverflow || !this.overflowEl) {
      return;
    }

    this.overflowEl.style.overflowY =
      val <= this.breakpoints.topper ? 'auto' : 'hidden';
  }
  ...
  public isPanePresented(): boolean {
    ...
    return wrappers.find((item) => item.contains(<HTMLElement>this.selector))
      ? true
      : false;
  }
  ...
  private getModuleRef(className): string {
    return (className.charAt(0).toLowerCase() + className.slice(1)).replace(
      'Module',
      '',
    );
  }
  ...
  public async moveToBreak(
    ...
    await this.transitions.doTransition({
      type,
      translateY: this.breakpoints.breaks[val],
    });
